...
inline extern float matRead(Mat mat, u32 x, u32 y) {
  return mat.data[x * mat.c + y];
}

inline extern void matWrite(Mat mat, u32 x, u32 y, float val) {
  mat.data[x * mat.c + y] = val;
}

inline extern float *matView(Mat mat, u32 x, u32 y) {
  return &mat.data[x * mat.c + y];
}
...
extern inline void matAlloc(Mat *mat) {
  mat->data = malloc(sizeof(float) * mat->r * mat->c);
}

extern inline void matFree(Mat mat) {
  free(mat.data);
}
...
void transposeMat(Mat src, Mat dst) {
...
void reduceMat(Mat src, Mat dst, unsigned x, unsigned y) {
  u32 count = 0;
  for (u32 i = 0; i < src.r; i++) {
    if (i == y) continue;
    for (u32 j = 0; j < src.c; j++) {
